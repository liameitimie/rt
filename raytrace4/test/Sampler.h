 ...
 */
__device__ 
inline float3 SpGGX(float xi_1,float xi_2,float3 n,float a){
...
__device__
inline float PdfGGX(float3 h,float3 n,float a){
	float NoH=dot(n,h);
	float D=D_GGX(a,NoH);
	return D*NoH;
}
__device__
inline float DHToDL_R(float3 hr,float3 l){
	return 1/(4*dot(l,hr));
}
__device__
inline float DHToDL_T(float3 ht,float3 l,float3 v,float IOR){
	float d=dot(v,ht)+IOR*dot(l,ht);
	return abs(IOR*IOR*dot(l,ht))/(d*d);
}
//
///*
