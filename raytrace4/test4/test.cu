...
#define WIDTH 1024
#define HEIGHT 768
#define SAMPLE_PER_PIXCEL 32

float4 h_c[HEIGHT][WIDTH];
__device__ float4 d_c[HEIGHT*WIDTH];
...
void initScene(){
	...
	h_s[0].p={0,1,0};
	h_s[0].r=1;
	h_s[0].matrial.baseColor={0.9,0.9,0.9};
	h_s[0].matrial.metallic=0;
	h_s[0].matrial.roughness=0;
...
__global__
void RTKernel(object* d_s,int n,Camera cam,cudaTextureObject_t tex){
...
//		sL=n*0.5+0.5;break;
		...
		bsdf.init(p,n,v,obj.matrial);
		float3 K=bsdf.SampleBSDF(xi[0],xi[1],xi[2]);
		
		sK*=K;
		if(length(sK)==0) break;
		
		r.o=bsdf.P;
		r.d=bsdf.getSampleDir();
	}
	if(length(sL)<1e3) color+=sL/1;
}
...
//	CHECK(cudaMemset(d_c,0,sizeof(h_c)));
	...
	RTKernel<<<dim3(WIDTH,HEIGHT),SAMPLE_PER_PIXCEL>>>(d_s,n,cam,tex);
	CHECK_KERNEL();
	
	CHECK(cudaMemcpyFromSymbol(h_c,d_c,sizeof(h_c)));
